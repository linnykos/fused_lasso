...
for(i in 1:n.length){
  ...
  dualplot_suite(y, sqrt((log(n.vec[i])+1)*sigma),jump.mean.org*log(n.vec[i]),jump.location.org)
  dev.off()
...
for(i in 1:n.length){
  set.seed(10)
  y = generate.problem(n.vec[i], jump.location.org, jump.mean.org, sigma)
  png(paste("figures/simul_",date,"_lambda-sqrtlogn_n-",n.vec[i],".png",sep=""), width=8, height=3, units="in", res=300)
  dualplot_suite(y, sqrt((log(n.vec[i])+1)*sigma),jump.mean.org,jump.location.org)
  dev.off()
