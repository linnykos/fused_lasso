...
for(i in 1:n.length){
  ...
  res = list(dist.mat = dist.mat, lambda.mat = lambda.mat, mse.mat = mse.mat,
   haus.mat = haus.mat, jumps.mat = jumps.mat, haus.filter.mat = haus.filter.mat,
   jumps.filter.mat = jumps.filter.mat, setup = setup)
...
png(paste0("~/DUMP/truth_", DATE, ".png"), height = 5, width = 10, units = "in", res = 300)
...
png(paste0("~/DUMP/screening-plot_", DATE, ".png"), height = 5, width = 10, 
 units = "in", res = 300)

par(mfrow = c(1,2))

plot(x = n.vec, y = lambda.vec, pch = 16, cex = 2, xlab = "Length of vector (n)",
     ylab = "Mean Lambda value (1-se rule, 5-fold cv)", ylim = c(0, max(lambda.vec+lambda.std)))
...
mse.vec = apply(res$mse.mat, 1, mean)
mse.std = apply(res$mse.mat, 1, sd)

plot(x = n.vec, y = mse.vec, pch = 16, cex = 2, xlab = "Length of vector (n)",
     ylab = "Mean MSE value (1-se rule, 5-fold cv)", ylim = c(0, max(mse.vec + mse.std)))
